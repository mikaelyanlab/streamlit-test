...
# Function to search web for new reports and extract species using Google with rotation
def trawl_for_reports():
    if 'logs' not in st.session_state:
        st.session_state.logs = []
    st.session_state.logs.append(f"Starting trawl at {datetime.now()}")
    global gdf
    query = "termite infestation North Carolina"
    headers = {
    ...
    try:
        response = requests.get(f"https://www.google.com/search?q={requests.utils.quote(query)}&num=20&hl=en&gl=us&tbs=qdr:y", headers=headers)
        ...
        for g in soup.find_all('div', class_='g'):
            ...
            if link and 'href' in link.attrs:
                ...
                if actual_url.startswith('http') and 'termite' in actual_url.lower() and 'google' not in actual_url.lower():
                    new_links.append(actual_url)
                    st.session_state.logs.append(f"Found link: {actual_url}")
                    # Extract title and snippet for species and county
                    title = g.find('h3').text.lower() if g.find('h3') else ''
                    snippet_div = g.find('div', {'data-snippet-id': True})
                    snippet = snippet_div.text.lower() if snippet_div else ''
