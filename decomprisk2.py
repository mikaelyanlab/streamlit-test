...
def fire_intensity_proxy(F, wind, T, theta):
    F = np.maximum(F, 1e-6)
    phi_w = 1.0 + 0.04 * np.clip(wind, 0, 25)
    dryness = 1.0 + np.clip((0.35 - theta), 0, 0.35) * 2.0
    heat = 1.0 + 0.02 * np.clip(T - 20.0, 0, 20)
    return (F ** 1.5) * phi_w * dryness * heat

def usle_erosion(R, SOM, LS=1.2, C_base=0.3, P=1.0, F=None, theta=None, B=None):
    K0 = 0.032
    K = K0 * np.exp(-2.0 * np.clip(SOM, 0, 0.1)/0.1)
    if F is None: F = 20.0
    if theta is None: theta = 0.25
    if B is None: B = 50.0
    cover_bonus = 0.5 * (theta/0.35) + 0.5 * (B/100.0)
    cover_bonus = np.clip(cover_bonus, 0, 1.2)
    mulch_effect = 1.0 / (1.0 + 0.02 * np.clip(F, 0, 50))
...
def simulate_once(T, P, RH, wind, L, ET, Qd, params: SimParams, dt=1.0):
    ...
    for t in range(1, n):
        ...
        dtheta = (P[t-1] - ET[t-1] - Qd[t-1]) / (params.Z * params.FC * 1000.0)
...
def synthetic_climate(n_days, climate="temperate", seed=0):
    ...
    if climate == "arid":
        ...
        P = np.clip(r.gamma(1.2, 2.0, n_days), 0, None)
    ...
    vpd = vpd_from_T_RH(T, RH)
    ET = np.clip(1.2 + 0.12*T + 0.8*vpd + 0.2*wind, 0, None)
    Qd = np.clip(0.1 * np.maximum(P - 15, 0), 0, None)
    if climate == "arid": L = np.full(n_days, 0.03)
...
with col2:
    ...
    if upl is not None:
        ...
        n = min(len(drv), n_days)
        T = drv["T_C"].values[:n]
        P = drv["P_mm"].values[:n]
        RH = drv["RH_%"].values[:n]
        wind = drv["Wind_ms"].values[:n]
        ET = drv["ET_mm"].values[:n]
        Qd = drv["Q_mm"].values[:n]
        L = drv["L_kgm2"].values[:n]
    else:
    ...
    def summarize_mc(mc_list, col):
        ...
        return np.percentile(M, [5,50,95], axis=0)
    ...
    erosion_avoided = sed_avoided_tons * per_ton_sed
    total_avoided = fire_avoided + erosion_avoided
    ...
    with colC:
        ...
        st.metric("Avoided Erosion Cost ($)", f"{erosion_avoided:,.0f}")
    with colD:
    ...
    def band_plot(x, q, median_name, title, ylab):
        ...
        fig.add_trace(go.Scatter(x=x, y=q[0], mode="lines", line=dict(width=0), showlegend=False))
        fig.add_trace(go.Scatter(x=x, y=q[2], mode="lines", fill="tonexty", name="5–95% band",
    ...
    payout = payout_from_dfi(dfi, tier=(dfi_t1, dfi_t2, dfi_t3), amounts=(pay1, pay2, pay3)) if red_flag else 0
    ...
    summary = {
        "climate": climate, "biome": biome, "days": n_days, "B": B, "I_idx": I_idx,
        "F0": F0, "theta0": theta0, "SOM0": SOM0, "Rm": Rm, "k0": k0, "Q10": Q10,
        "Z": Z, "FC": FC, "MC_draws": n_mc, "Avoided_Fire_$": fire_avoided,
        "Avoided_Erosion_$": erosion_avoided, "Total_Avoided_$": total_avoided,
        "DFI": dfi, "RedFlag": red_flag, "Payout_$": payout
    }
    ...
    term_lines = [
        ...
        f"Avoided losses (toy): Fire ${fire_avoided:,.0f}, Erosion ${erosion_avoided:,.0f}, Total ${total_avoided:,.0f}",
...
st.markdown("---")
st.caption("Notes: Research/education model. Fire & erosion conversions are placeholders; calibrate with local data.")
