...
O2k, H2k, pHr = radial_profile(
    core.O2_uM, core.H2_kPa, core.pH, r_rel, k_r, uM_to_kPa
)
...
with tab1:
    ...
    p1 = alt.Chart(df_plot).mark_line().encode(x='x', y='pH', title='pH')
    p2 = alt.Chart(df_plot).mark_line(color='black').encode(x='x', y='O2_kPa', title='O₂ kPa')
    p3 = alt.Chart(df_plot).mark_line(color='green').encode(x='x', y='H2_kPa', title='H₂ kPa')
    p4 = alt.Chart(df_plot).mark_line(color='gray').encode(x='x', y='Eh', title='Eh mV')
    st.altair_chart((p1 & p2 & p3 & p4).resolve_axis(y='independent'), use_container_width=True)

with tab2:
    st.subheader("Radial slice")
...
c2.metric("O2 shell δ", f"{r1:.0f} µm")
...
if energy_ok:
    ...
    st.error("✖ Shortfall – adjust inputs")

with st.expander("Download CSVs"):
    st.download_button("Axial", df_ax.to_csv(index=False).encode(), "axial.csv", "text/csv")
