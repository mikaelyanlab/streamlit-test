...
def methane_oxidation(C, t, C_atm, g_s, Vmax_ref, Km_ref, Pi, O2_ext, T, k_L, V_cell, scaling_factor, k_O2):
    ...
    J_CH4 = k_L * (C_cyt_eq - C_cyt)  # CH4 transfer

    # Oxygen influx (new)
    O2_eq = g_s * O2_ext  # driving force based on stomatal conductance
    J_O2 = k_O2 * (O2_eq - O2_cyt)
    ...
    dO2_dt = J_O2 - V_MMO  # includes influx

    return [dC_cyt_dt, dCH3OH_dt, dO2_dt]

# Streamlit UI
st.title("Methane Oxidation Model with Enzyme Sensitivity and O₂ Influx")
...
k_O2 = st.sidebar.slider("O₂ Transfer Coefficient (k_O2, m/s)", 0.001, 0.1, 0.01)
...
C0 = [0.2, 0.1, O2_ext]  # Initial [CH4_cyt, CH3OH, O2_cyt]
sol = odeint(methane_oxidation, C0, time, args=(C_atm, g_s, Vmax_ref, Km_ref, Pi, O2_ext, T, k_L, V_cell, scaling_factor, k_O2))
...
fig_gauge = go.Figure(go.Indicator(
    ...
    gauge={
        'axis': {'range': [0, 0.3]},
        'bar': {'color': "#ffcc00"},
        'steps': [{'range': [0, 0.3], 'color': "#ffeeee"}],
