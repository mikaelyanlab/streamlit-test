...
def methane_oxidation(C, t, g_s, Vmax, Km, Pi, J_ETC, n_MMO, H_CH4, k_L):
...
H_CH4 = st.sidebar.slider("Methane Henry's Law Constant (H_CH4, mmol/L·atm)", 0.5, 2.0, 1.4)
k_L = st.sidebar.slider("Mass Transfer Coefficient (k_L, m/s)", 0.001, 0.1, 0.01)

# Solve ODEs
time = np.linspace(0, 100, 500)
C0 = [0.5, 0.2, 0.1]  # Initial concentrations
sol = odeint(methane_oxidation, C0, time, args=(g_s, Vmax, Km, Pi, J_ETC, n_MMO, H_CH4, k_L))
