...
# ---------------------------------------------------------
# 1. DATA (your full embedded JSON exactly as provided)
# ---------------------------------------------------------
DATA_JSON = r"""
[
  {
...
"""
# NOTE: For space, replace `[ ... YOUR ENTIRE JSON CONTENT ... ]`
# with your original JSON block exactly as you provided it.
...
THEME_KEYWORDS = {
    "Feedback": {
        "feedback","cybernetics","flow","homeostasis","neg-entropy","regulation",
        "stability","trophallaxis"
    },
    "Constraint": {
        "constraint","constraint closure","canalization","boundary","autonomy",
        "architecture","individuality"
    },
    "Evolution": {
        "evolution","rewiring","innovation","divergence","convergence",
        "major transitions","multilevel"
    },
    "Symbiosis": {
        "symbiosis","holobiont","mutualism","pathogen","microbe","microbiome",
        "syntrophy","commensal"
    },
    "Robustness": {
        "robustness","fragility","resilience","vulnerability","failure",
        "redundancy","buffering"
    },
    "Resource Flux": {
        "nitrogen","carbon","resource","amino acid","fermentation","redox","gas flux"
    }
}

def assign_themes(keywords):
    """Return all conceptual themes that match the keyword set."""
    ks = set(keywords)
    hits = {theme for theme, kws in THEME_KEYWORDS.items() if ks & kws}
    return sorted(hits)
...
st.caption("ENT-591/791-006 • Spring 2026 • Tuesdays & Thursdays • 1:30 to 2:45 • 01406 Gardner Hall")
st.caption("Dr. Aram Mikaelyan • Entomology and Plant Pathology (NCSU)")
st.caption("""
This course examines insects and their microbes as integrated biological systems, moving from individual feedback
and flow to holobiont organization, pathogenic disruption, and evolutionary transformation of constraint networks.
Students analyze how resource regimes, ecological pressures, and long-term system dynamics shape convergence and
divergence across insect–microbe lineages.
""")
...
with st.sidebar:
    ...
    color_mode = st.selectbox("Color nodes by:", ["Module", "Theme"])
...
with tab_graph:
    ...
    # Filter subgraph by selected themes (if any)
    if selected_theme:
    ...
    def node_color(d):
        if color_mode == "Module":
            ...
            if d["themes"]:
                return theme_palette.get(d["themes"][0], "#999")
            return "#999"


    # ---------------------------------------------------------
    # 5. Render Network via PyVis
    # ---------------------------------------------------------
