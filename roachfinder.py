...
if restaurants_upload and violations_upload and st.session_state.merged_df is None:
    try:
        ...
        merged_df = violations_df.merge(restaurants_df[['attributes.HSISID', 'attributes.NAME', 'attributes.ADDRESS1', 'attributes.CITY', 'attributes.POSTALCODE', 'attributes.X', 'attributes.Y']], 
                                      on='attributes.HSISID', how='left')
...
if merged_df is not None and not merged_df.empty:
    ...
    st.warning("No cockroach-related violations found or data not uploaded yet. Please upload CSV/JSON files.")
...
if merged_df is not None and not merged_df.empty:
    ...
    st.write("No data available for mapping. Please upload files to proceed.")
