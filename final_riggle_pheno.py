...
V_cell = 1e-15  # L; typical plant cell volume (placeholder; not used directly)

# --- ODE System ---
def methane_oxidation(C, t, C_atm, O2_atm, g_s, Vmax_ref, Km_ref, Pi, T,
                      k_L_CH4, k_L_O2, V_cell, photosynthesis_on, expression_percent):
    C_cyt, CH3OH, O2_cyt = C
    T_K = T + 273.15

    # Vmax and Km adjustments
    Vmax_T = Vmax_ref * (expression_percent / 100.0) * np.exp(-E_a / R * (1.0/T_K - 1.0/T_ref))
    Km_T = Km_ref * (1.0 + 0.02 * (T - 25.0))
    Vmax = Vmax_T * np.exp(-0.02 * (Pi / 100.0))

    k_MeOH = k_MeOH_ref * np.exp(-E_a_MeOH / R * (1.0/T_K - 1.0/T_ref))

    # Henry’s constants with temperature and osmolarity adjustments (clamped to avoid negatives)
    alpha, beta = 0.02, 0.01
    osm_factor = max(0.0, 1.0 - beta * Pi)
    H_CH4 = H_0_CH4 * np.exp(-alpha * (T - 25.0)) * osm_factor
    H_O2  = H_0_O2  * np.exp(-alpha * (T - 25.0)) * osm_factor

    # Partial pressures (atm)
    P_CH4 = (C_atm / 1e6)    # ppm -> mole fraction
    P_O2  = (O2_atm / 100.0) # % -> fraction

    # Equilibrium concentrations at the interface (mmol/L_cyt)
    C_cyt_eq = H_CH4 * P_CH4
    O2_eq    = H_O2  * P_O2

    # Fluxes scaled by relative stomatal conductance (effective exchange)
    g_s_scale = g_s / g_s_ref
    J_CH4 = k_L_CH4 * g_s_scale * (C_cyt_eq - C_cyt)
    J_O2  = k_L_O2  * g_s_scale * (O2_eq    - O2_cyt)

    # MMO activity (dual-substrate Michaelis–Menten)
    Km_O2 = 0.001  # mmol/L
    ...
    if photosynthesis_on and O2_cyt < O2_eq:
        O2_prod_eff = 0.005  # mmol/L_cyt/s
    else:
        O2_prod_eff = 0.0
...
cytosol_fraction = st.sidebar.slider("Cytosol Fraction (% of cell volume)", 1, 100, 5) / 100.0
# active_volume = V_cell * cytosol_fraction  # defined but not used in this intracellular model
...
alpha, beta = 0.02, 0.01
osm_factor = max(0.0, 1.0 - beta * Pi)
O2_init = H_0_O2 * np.exp(-alpha * (T - 25.0)) * osm_factor * (O2_atm / 100.0)
C0 = [0.0001, 0.0001, O2_init]

# Solve ODEs
def wrapped_ode(t, C):
    return methane_oxidation(
        C, t, C_atm, O2_atm, g_s, Vmax_ref, Km_ref, Pi, T,
        k_L_CH4, k_L_O2, V_cell, photosynthesis_on, expression_percent
    )
...
for r in (1, 2, 3):
    fig_plots.update_yaxes(title_text="Concentration (mmol/L_cyt)", row=r, col=1)

# Final MMO rate (for gauge) — intracellular basis
C_cyt_final = sol[-1, 0]
O2_cyt_final = sol[-1, 2]
Km_T = Km_ref * (1.0 + 0.02 * (T - 25.0))
Vmax_T = Vmax_ref * (expression_percent / 100.0) * np.exp(-E_a / R * (1.0/(T + 273.15) - 1.0/T_ref))
Vmax_osm = Vmax_T * np.exp(-0.02 * (Pi / 100.0))
Km_O2 = 0.001
...
fig_gauge = go.Figure(go.Indicator(
    mode="gauge+number",
    value=V_MMO_final,
    number={'suffix': " mmol·L_cyt⁻¹·s⁻¹", 'valueformat': '.10g'},
    title={'text': "Final CH₄ Oxidation Rate (intracellular)"},
    gauge={
        'axis': {'range': [0, 1e-6]},
        'bar': {'color': "#ffcc00"},
...
if st.button("Run Sensitivity Analysis"):
    ...
    for val in param_range:
        ...
        local_expr = expression_percent if selected_param != "expression_percent" else val
        ...
        # Consistent initial O2 with clamped osmolarity factor
        alpha_s, beta_s = 0.02, 0.01
        osm_factor_s = max(0.0, 1.0 - beta_s * local_Pi)
        local_O2_init = H_0_O2 * np.exp(-alpha_s * (local_T - 25.0)) * osm_factor_s * (local_O2_atm / 100.0)
        ...
        sol_local = solve_ivp(
            fun=lambda t, C: methane_oxidation(
                ...
                local_Pi, local_T, local_k_L_CH4, local_k_L_O2, V_cell, photosynthesis_on, local_expr
            ),
            t_span=(time[0], time[-1]),
            y0=local_C0,
            t_eval=time,
        ...
        local_Km_T = local_Km_ref * (1.0 + 0.02 * (local_T - 25.0))
        local_Vmax_T = local_Vmax_ref * (local_expr / 100.0) * np.exp(-E_a / R * (1.0/(local_T + 273.15) - 1.0/T_ref))
        local_Vmax_osm = local_Vmax_T * np.exp(-0.02 * (local_Pi / 100.0))
        Km_O2_local = 0.001
    ...
    df = pd.DataFrame(results, columns=[param_info["label"], "Final CH₄ Oxidation Rate (mmol·L_cyt⁻¹·s⁻¹)"])
    fig_sa = go.Figure()
    fig_sa.add_trace(go.Scatter(x=df[param_info["label"]],
                                y=df["Final CH₄ Oxidation Rate (mmol·L_cyt⁻¹·s⁻¹)"],
                                mode="lines+markers"))
    fig_sa.update_layout(title=f"Sensitivity: {param_info['label']} vs. Final CH₄ Oxidation Rate",
                         xaxis_title=param_info["label"], yaxis_title="mmol·L_cyt⁻¹·s⁻¹")
...
- **V_cell**: 1e-15 L (Typical plant cell volume; placeholder)
""")

st.subheader("Key Equations")
st.latex(r"Vmax_T = Vmax_{ref} \times \exp\left(-\frac{E_a}{R}\left(\frac{1}{T_K}-\frac{1}{T_{ref}}\right)\right)")
st.latex(r"Vmax = Vmax_T \times \exp(-0.02 \times (Pi / 100))")
st.latex(r"Km_T = Km_{ref} \times (1 + 0.02 \times (T - 25))")
st.latex(r"H = H_0 \times \exp(-\alpha (T - 25)) \times \max(0, 1 - \beta \, Pi)")
...
st.latex(r"\frac{dO2_{cyt}}{dt} = J_{O2} - V_{MMO} + O2_{prod}\ \text{(only if } O2_{cyt}<O2_{eq}\text{)}")

# Debug output
k_MeOH_scaled = k_MeOH_ref * np.exp(-E_a_MeOH / R * (1.0/(T + 273.15) - 1.0/T_ref))
