...
def methane_oxidation(C, t, C_atm, O2_atm, g_s, Vmax_ref, Km_ref, Pi, T,
    ...
    # Vmax and Km (temp + expression + osmolarity)
    Vmax_T = Vmax_ref * (expression_percent / 100.0) * np.exp(-E_a / R * (1.0/T_K - 1.0/T_ref)) * np.exp(-k_den * (T_K - T_opt)**2)
...
elif Vmax_ref <= 0: err = "Vmax_ref must be > 0."
elif k_L_CH4 <= 0 or k_L_O2 <= 0: err = "Mass transfer coefficients must be > 0."
elif O2_atm <= 0: err = "Atmospheric O₂ must be > 0."
elif C_atm <= 0: err = "Atmospheric CH₄ must be > 0."
elif g_s <= 0: err = "Stomatal conductance must be > 0."
elif T < -273.15: err = "Temperature must be above -273.15°C."
if err:
    st.error(err)
    st.stop()
...
T_opt = 308.15  # 35°C in Kelvin
k_den = 0.01    # Denaturation rate constant
Vmax_T = Vmax_ref * (expression_percent / 100.0) * np.exp(-E_a / R * (1.0/(T + 273.15) - 1.0/T_ref)) * np.exp(-k_den * ((T + 273.15) - T_opt)**2)
...
if st.button("Run Sensitivity Analysis"):
    ...
    for v in vals:
        ...
        sol_local = solve_ivp(
            fun=lambda t, C: methane_oxidation(
                C, t,
                local["C_atm"], local["O2_atm"], local["g_s"],
                local["Vmax_ref"], local["Km_ref"], local["Pi"], local["T"],
                local["k_L_CH4"], local["k_L_O2"],
                photosynthesis_on, local["expression_percent"]
            ),
            t_span=(time[0], time[-1]),
        ...
        Km_T_loc = local["Km_ref"] * (1.0 + 0.02 * (local["T"] - 25.0))
        T_opt = 308.15  # 35°C in Kelvin
        k_den = 0.01    # Denaturation rate constant
        Vmax_T_loc = local["Vmax_ref"] * (local["expression_percent"] / 100.0) * np.exp(-E_a / R * (1.0/(local["T"] + 273.15) - 1.0/T_ref)) * np.exp(-k_den * ((local["T"] + 273.15) - T_opt)**2)
        Vmax_osm_loc = Vmax_T_loc * np.exp(-0.02 * (local["Pi"] / 100.0))
        V_MMO_f_loc = Vmax_osm_loc * (C_cyt_f / (Km_T_loc + C_cyt_f)) * (O2_cyt_f / (Km_O2 + O2_cyt_f))
    ...
    fig_line.add_trace(go.Scatter(x=df_line[info["label"]],
                                  ...
                                  mode="lines+markers"))
    fig_line.update_layout(title=f"Sensitivity: {info['label']} vs. Final CH₄ Oxidation Rate",
                           xaxis_title=info["label"], yaxis_title="mmol/L/s")
    st.plotly_chart(fig_line)
    # CSV for the single sweep
    st.download_button(
        "Download CSV",
    ...
    for param, pinfo in param_options.items():
        ...
        # Normalize per-parameter for the heatmap
