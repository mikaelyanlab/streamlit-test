# app.py — Insect–Microbe Systems Course Network (clickable version)
# -------------------------------------------------------------------
...
with tab_graph:
    ...
    # Capture user input
    selected_node = st.text_input("Click a node label and type its ID here to view details:")

    if selected_node and selected_node in G.nodes:
        d = G.nodes[selected_node]
        ...
        st.caption("Click a node label in the plot and type its ID above to see full session details.")
